...
log_head "6/7  Panel styling (red/maroon accents, rounded corners)"
...
if priv apt-get install -y picom 2>/dev/null || priv apt-get install -y compton 2>/dev/null; then
    ...
    if "$COMPOSITOR_BIN" --help 2>&1 | grep -qi "animation"; then
        ...
        log_warn "you're running a build that understands them (see the optional log_head below)."
    fi
...
if ask "Install Whisker Menu (Mint-style application menu, added alongside your current menu)?" "N"; then
    priv apt-get install -y xfce4-whiskermenu-plugin \
        && xfce4-panel --add=whiskermenu 2>/dev/null \
        && log_ok "Whisker Menu added — right-click to reposition, remove the old Applications Menu if you don't want both." \
        || log_warn "Whisker Menu install/add failed — add it manually via Panel → Add New Items."
fi
...
if ask "Add CPU + network graph plugins to the panel (cpugraph, netload)?" "N"; then
    priv apt-get install -y xfce4-cpugraph-plugin xfce4-netload-plugin \
        && xfce4-panel --add=cpugraph 2>/dev/null \
        && xfce4-panel --add=netload 2>/dev/null \
        && log_ok "cpugraph + netload added to the panel — right-click either to reposition or restyle." \
        || log_warn "Couldn't add them automatically — install succeeded, add via Panel → Add New Items."
fi
