...
#
#  21-theme-tokyonight.sh handles everything *inside* the session.
...
if ask "Install the Tokyo Night Plymouth splash (shown while the kernel boots)?"; then
    priv apt-get install -y plymouth plymouth-themes || log_warn "Plymouth package install had issues — continuing."

    if git clone --depth=1 https://github.com/thedustin/plymouth-theme-tokyonight.git "$WORK_DIR/plymouth" 2>/tmp/tokyonight-plymouth-clone.log; then
        # The repo may have various directory layouts; find the actual theme dir.
        PLYMOUTH_THEME_DIR=""
        for candidate in "$WORK_DIR/plymouth" "$WORK_DIR/plymouth/themes/tokyonight" "$WORK_DIR/plymouth/theme"; do
            if [[ -d "$candidate" ]] && find "$candidate" -maxdepth 1 -name "*.plymouth" 2>/dev/null | head -1 | grep -q .; then
                PLYMOUTH_THEME_DIR="$candidate"
                break
            fi
        done
        if [[ -z "$PLYMOUTH_THEME_DIR" ]]; then
            PLYMOUTH_THEME_DIR=$(find "$WORK_DIR/plymouth" -maxdepth 3 -name "*.plymouth" 2>/dev/null | head -1 | xargs -r dirname)
        fi

        if [[ -n "$PLYMOUTH_THEME_DIR" && -d "$PLYMOUTH_THEME_DIR" ]]; then
            THEME_BASENAME=$(basename "$PLYMOUTH_THEME_DIR")
            priv mkdir -p /usr/share/plymouth/themes
            priv cp -r "$PLYMOUTH_THEME_DIR" "/usr/share/plymouth/themes/$THEME_BASENAME"
            log_ok "Theme files copied to /usr/share/plymouth/themes/$THEME_BASENAME"

            APPLIED=0
            if command -v plymouth-set-default-theme &>/dev/null; then
                if priv plymouth-set-default-theme -R "$THEME_BASENAME" 2>/tmp/plymouth-set-theme.log; then
                    APPLIED=1
                fi
            fi
            if [[ $APPLIED -eq 0 ]]; then
                PLYMOUTH_PLY=$(find "/usr/share/plymouth/themes/$THEME_BASENAME" -maxdepth 1 -name "*.plymouth" | head -1)
                if [[ -n "$PLYMOUTH_PLY" ]]; then
            ...
            if [[ $APPLIED -eq 1 ]]; then
                log_ok "Plymouth theme set to $THEME_BASENAME."
            ...
            log_warn "Cloned tokyonight-plymouth but couldn't find a valid .plymouth file — upstream layout may have changed."
        fi
    else
        log_warn "Clone failed — check your network/DNS. Log: /tmp/tokyonight-plymouth-clone.log"
    fi
else
...
elif ask "Install the Tokyo Night GRUB background (only matters if you actually see the GRUB menu)?"; then
    GRUB_DEFAULT="/etc/default/grub"
    WALLPAPER_DIR="/usr/share/backgrounds/xfce/devuan-tokyonight"
    GRUB_BG=$(ls "$WALLPAPER_DIR"/*.{jpg,jpeg,png} 2>/dev/null | head -1)
    if [[ -n "$GRUB_BG" ]]; then
        # Preserve the file extension
        GRUB_BG_EXT="${GRUB_BG##*.}"
        GRUB_BG_DEST="/boot/grub/devuan-tokyonight.$GRUB_BG_EXT"
        ...
        log_warn "No wallpaper found in $WALLPAPER_DIR — run 21-theme-tokyonight.sh first."
    fi
else
...
if dpkg -l lightdm-gtk-greeter 2>/dev/null | grep -q '^ii'; then
    if ask "Theme the LightDM login screen to match (copies your already-installed GTK theme system-wide)?"; then
        ...
        if [[ -d "/usr/share/themes/$GTK_THEME_NAME" ]]; then
            ...
            log_warn "Tokyonight-Dark-BL not found in /usr/share/themes or ~/.themes — run 21-theme-tokyonight.sh first."
        fi

        ICON_THEME_NAME=""
        for candidate in "Tela-circle-dark" "Tela-circle-tahoe" "Papirus-Dark"; do
