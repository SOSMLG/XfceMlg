...
if is_installed timeshift; then
    ...
    log_warn "One-time setup needed: run 'timeshift-launcher' as root (doas/sudo)"
    log_warn "or find Timeshift in the app menu to choose rsync vs BTRFS mode, where"
    log_warn "snapshots are stored, and a schedule. That choice is left to you rather"
    log_warn "than guessed automatically."
else
