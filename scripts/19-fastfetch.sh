#!/usr/bin/env bash
# DEBSWAY_DESC: minimal fancy fastfetch + Mocha btop
# DEBSWAY_DEFAULT: Y
#  19-fastfetch.sh — system info on terminal open
#  Writes one minimal, fancy fastfetch config locally (small Debian
#  logo, Catppuccin-red keys, essential modules only — no network
#  needed beyond the fastfetch package itself). Also offers btop
#  (system monitor) with just the Catppuccin Mocha flavor.
#  Privilege: priv() (doas-first, sudo fallback) (fastfetch/btop/curl installs)
set -uo pipefail
...
if ask "Write the minimal fancy fastfetch config (small logo, red keys, essentials only)?"; then
...
log_head "3/3  btop (system monitor) — Catppuccin Mocha"
if ask "Install btop with the Catppuccin Mocha theme (matches the Red/Black desktop)?"; then
