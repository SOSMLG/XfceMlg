...
pkg gimp optional
pkg picom optional
pkg xfce4-whiskermenu-plugin optional
...
# Neovim must be gone (VSCodium is the editor now).
if command -v nvim >/dev/null 2>&1 || is_installed neovim; then
    report "neovim retired" warn "still present — re-run 40-vscodium.sh to purge"
else
    report "neovim retired" ok "absent, codium is the editor"
fi
# Picom: square compositor config (corner-radius 0, no blur).
if [ -f "$HOME/.config/picom.conf" ]; then
    if grep -qE 'corner-radius\s*=\s*0' "$HOME/.config/picom.conf" 2>/dev/null \
        && grep -qE 'blur-method\s*=\s*"none"' "$HOME/.config/picom.conf" 2>/dev/null; then
        report "picom square config" ok "corner-radius 0, no blur"
    else
        report "picom square config" warn "present but not square/no-blur (re-run 21-theme-catppuccin.sh)"
    fi
else
    report "picom square config" warn "missing (re-run 21-theme-catppuccin.sh)"
fi
# Panel: square CSS (no rounded 16px) + no duplicate curated plugins.
if [ -f "$HOME/.config/gtk-3.0/gtk.css" ]; then
    if grep -q "border-radius: 16px" "$HOME/.config/gtk-3.0/gtk.css" 2>/dev/null; then
        report "panel square CSS" warn "still rounded (re-run 21-theme-catppuccin.sh)"
    else
        report "panel square CSS" ok "square"
    fi
else
    report "panel square CSS" warn "missing (re-run 21-theme-catppuccin.sh)"
fi
...
# Service name is lowercase on OpenRC (Devuan), capital on systemd.
if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
    service_state NetworkManager NetworkManager optional
else
    service_state network-manager NetworkManager optional
fi
# No unmanaged wifi radios (the minimal-install "device not managed" trap).
