...
if [ -n "$local_links" ]; then
    ...
    if ask_no_full "Remove these dead symlinks?"; then
        while IFS= read -r lnk; do
            [ -z "$lnk" ] && continue
...
# Never auto-fires on --full (large, potentially surprising): ask_no_full
# honors the default N even when DEBSWAY_FULL is set.
if ask_no_full "Run 'apt full-upgrade' (upgrade all installed packages)? This can be large. (y/N)?" "N"; then
    apt_update
