...
SCRIPTS=(
    ...
    "catppuccinTheme.sh|Install the Catppuccin (Black+Red) GTK/xfwm4 theme, icons, cursors, panel CSS|Y"
    "picomSetup.sh|Install picom for lightweight fade animations (battery-friendly: no shadows/blur, unredirects fullscreen)|Y"
    "touchpadTrackpointFix.sh|Apply touchpad/trackpoint polling + libinput fixes|Y"
    "hardwareSupport.sh|Install WiFi/Bluetooth firmware, CPU microcode, and fwupd|Y"
    ...
    "firewallSetup.sh|Enable ufw with a deny-incoming/allow-outgoing baseline (SSH-safe)|Y"
    "installFonts.sh|Install Noto, Font Awesome, and JetBrainsMono Nerd Font|Y"
    "terminalRedTheme.sh|Recolor xfce4-terminal to Catppuccin Red (optional Alacritty/Kitty)|Y"
    "terminalButterbash.sh|Install ButterBash + XFCE-specific shell additions|Y"
    "fastfetchConfig.sh|Install fastfetch + your curated config presets|Y"
    "usefulApps.sh|Install base tools, Python/data-science stack, Geany, VLC|Y"
    "mintStyleApps.sh|Install Mint-style everyday apps from Debian's own repos (Ristretto, Atril, GNOME Disks)|Y"
    "desktopEssentials.sh|Set up Flatpak, printing, GParted, and the GUFW firewall panel|Y"
    ...
    "btopSetup.sh|(optional) Install btop with official Catppuccin theming|N"
